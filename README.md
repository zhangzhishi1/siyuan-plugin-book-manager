# Book Manager

Keep your library, reading progress, and notes in one connected workflow. Book Manager can retrieve metadata by ISBN, manage local covers and ebook files, and write a reusable book card into a SiYuan document you choose.

## Highlights

- **Fast book entry**: look up title, author, publisher, publication date, cover, and tags by ISBN using Douban or ISBN Search.
- **Complete library tracking**: manage reading status, progress, ratings, categories, tags, and personal notes.
- **Flexible browsing**: switch between list, table, and grid views with multi-keyword search, filters, and sorting.
- **SiYuan document linking**: connect an existing document and open it directly from the book; categories can follow the document's position in the SiYuan tree.
- **Updatable book cards**: write the cover, metadata, source-file links, and notes into the linked document. Updates replace only plugin-managed blocks and preserve your own writing.
- **Library overview**: review reading totals, status distribution, average rating, categories, and common tags in the dock.

## Getting Started

1. Install and enable the plugin, then open it from the book icon in the top bar.
2. Add a book manually or enter an ISBN to retrieve its metadata.
3. Attach local PDF, EPUB, or other source files, or save external links.
4. Link an existing SiYuan document and select "Update Note" to write or refresh its book card.

## Basic and Pro Editions

### Basic Edition

The Basic Edition covers manual personal-library management:

- Manually add, edit, and delete books
- Track reading status, progress, ratings, categories, tags, and personal notes
- Browse the library in list, table, and grid views
- Search, filter, and sort by keywords, reading status, categories, and tags

### Pro Edition

The Pro Edition includes everything in Basic, plus automation and SiYuan note integration:

- Multi-provider ISBN lookup for title, author, publisher, publication date, cover, and tags
- Cover, PDF, EPUB, MOBI, AZW3, TXT, and external source-link management
- Existing SiYuan document linking and book cards with metadata, source links, and notes
- One-click checking and updating of linked notes while preserving user-written content
- Document-tree category sync, reading statistics, category distribution, and tag overview

### Pricing

- Six months: ¥29.9
- One year: ¥60 list price, ¥50 at 15% off

After purchasing Pro, paste the license code into the plugin to activate Pro features.

## Requirements

- SiYuan 3.7.0 or later
- Windows, macOS, or Linux desktop
- Network access for ISBN lookup and license activation
